...
if current_page == "industry_explore":
    ...
    for idx, d in enumerate(preset_directions):
        with cols[idx % 4]:
            if st.button(f"🔥 {d}", key=f"preset_btn_{idx}", use_container_width=True):
                selected_preset = d

    user_industry = st.text_input("或者自定义输入你想探索的行业方向：", value=selected_preset if selected_preset else "AI")

    if st.button("🚀 生成行业全景导航", type="primary", use_container_width=True, key="gen_industry_btn"):
    ...
    if "nav_data" in st.session_state and "error" not in st.session_state["nav_data"]:
        ...
        for idx, pos in enumerate(positions):
            pos_title = pos.get('title', '未知岗位')
            with st.container():
                st.markdown(f"""
                <div class="metric-card">
                    <h3>📌 {pos_title} <span style="font-size:0.8rem; color:#64748b; float:right;">推荐度: {pos.get('recommendation', {}).get('score')} | 难度: {pos.get('difficulty')}</span></h3>
                ...
                with col_btn1:
                    if st.button(f"🔍 深度剖析此岗位", key=f"ind_pos_detail_{idx}_{pos_title}", use_container_width=True):
                        st.session_state["selected_position"] = pos_title
                        st.session_state["page"] = "position_detail"
                        st.rerun()
                with col_btn2:
                    if st.button(f"➕ 加入横向对比", key=f"ind_pos_compare_{idx}_{pos_title}", use_container_width=True):
                        if pos_title not in st.session_state["compare_list"]:
                            st.session_state["compare_list"].append(pos_title)
                            st.success(f"已成功将 {pos_title} 加入对比！")
...
# ----------------- 模块 B：职业地图 (Explore Tree) -----------------
    ...
    if "error" in tree_data:
        ...
        cols = st.columns(len(tree_data) if len(tree_data) > 0 else 1)
        for cat_idx, (category, sub_positions) in enumerate(tree_data.items()):
            with cols[cat_idx % len(cols)]:
                st.markdown(f"""
                <div style="background:#f1f5f9; padding:15px; border-radius:10px; text-align:center; font-weight:bold; margin-bottom:10px;">
                    ⚙️ {category}
                </div>
                """, unsafe_allow_html=True)
                for pos_idx, pos in enumerate(sub_positions):
                    if st.button(f"👉 {pos}", use_container_width=True, key=f"tree_btn_{cat_idx}_{pos_idx}_{pos}"):
...
# ----------------- 模块 C：岗位详情页 (Position) -----------------
    ...
    with col_t2:
        if st.button("➕ 加入对比", key="pos_detail_add_compare", use_container_width=True):
    ...
    if "error" in pos_data:
        ...
        st.markdown(f"## 💡 {pos_data.get('title', pos_name)}")
...
# ----------------- 模块 D：公司情报页 (Company) -----------------
    ...
    if st.button("🔍 查询公司情报", type="primary", key="search_company_btn"):
    ...
    if "error" in comp_data:
        ...
        st.markdown(f"# 🏢 {comp_data.get('name', st.session_state['selected_company'])}")
...
# ----------------- 模块 E：JD 高频技能分析 (JD) -----------------
    ...
    jd_input = st.text_area("在此粘贴岗位 JD 原文：", placeholder="例如：负责多模态大模型对齐与指令微调，熟悉 PyTorch...", height=150, key="jd_text_input_area")
    
    if st.button("🚀 一键深度解析 JD", type="primary", use_container_width=True, key="jd_analysis_btn"):
    ...
    if "jd_result" in st.session_state and "error" not in st.session_state["jd_result"]:
        ...
        st.markdown(f"## 📋 岗位分析结果：{jd_res.get('job_title', '综合岗位')}")
    ...
    if not compare_list:
        ...
        for idx, pos_title in enumerate(compare_list):
            with cols[idx]:
                ...
                if st.button(f"🗑️ 移除此岗位", key=f"compare_remove_btn_{idx}_{pos_title}", use_container_width=True):
